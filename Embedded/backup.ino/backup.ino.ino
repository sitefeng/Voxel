...
void readImageRequest() {
  ...
  for (int i=0; i<server.args(); i++) {
    String message = "<Message> NAME:"+server.argName(i) + "\n VALUE:" + server.arg(i) + "\n";
    Serial.print(message);

    String argName = server.argName(i);
    if (argName == "width") {
      imageWidth = server.arg(i).toInt;
    } else if (argName == "height") {
      imageHeight = server.arg(i).toInt;
  ...
  String commandId = client.readStringUntil('/');
  char comma = client.read();
  Serial.print(commandId);
  Serial.print(comma);
  imageWidth = client.readStringUntil('/').toInt();
  comma = client.read();
  imageHeight = client.readStringUntil('/').toInt();
  comma = client.read();

  String imgString = client.readStringUntil('\r');
  client.flush();
